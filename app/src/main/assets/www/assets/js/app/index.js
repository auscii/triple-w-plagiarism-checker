...
$('#btn-invitation-submit').click(function() {
...
function FETCH_CONFERENCES() {
	if (userType == mobile) {
		...
		database.ref(programmes + conferences).on('child_added', function(data) {
			...
			if (userStatus == 7) {
				MODAL('#modal-invitation-warning', 'open');
				NO_AVAILABLE_CONFERENCE();
			} else if (userStatus == uniqNum) {
				MODAL('#modal-invitation-prompt', 'open');
				NO_AVAILABLE_CONFERENCE();
			} else {
				if (key != undefined && userPaperCategories == category && userStatus == 1) {
...
function NO_AVAILABLE_CONFERENCE() {
	$('#main-mobile-progress-spinner').css({"display":"none"}); 
...
function FETCH_USERS() {
	...
	if (userType == web) {
		database.ref(users).on('value', function(snapshot) {
	    	$('#total-users').html(snapshot.numChildren());
		});
		database.ref(users).on('child_added', function(snapshot) {
			database.ref(users + snapshot.key).on('child_added', function(data) {
		        ...
		        var btnSendInvStatus = "enabled";
		        if (userStatus == 1 || userStatus == 69) {
...
function VIEW_PAPER(conferenceKey, conferenceBanner, conferenceEventTitle, conferenceEventPlace, conferenceEventDate, conferenceEventTime, 
	...
	$('#btn-papers-submit-check').click(function() {
		var btnReviewStatus = "none";
		if (userAccountType == paperReviewer) {
			btnReviewStatus = "block";
		}
...
function DOWNLOAD_REVIEW_PAPER(paperKey, paperSubmittedUrl, conferenceKey, conferenceTitle, conferenceBanner, action, subscriberKey) {
	...
	if (reviewPaperKey || reviewPaperKey != undefined) {
		...
		$('#btn-submit-review-paper').click(function() {
		    ...
		    INSERT_NOTIFICATIONS(reviewPaperConferenceKey, reviewPaperConferenceBanner, reviewPaperEventTitle, 
								 reviewPaperKey, processing, reviewAuthorPaper, reviewAuthorUserKey, 
								 userEmailAddress);
...
function CONTINOUS_CHECKING_STATUS_PR() {
	var status = false;
	var pr = setInterval( function() {
		...
		database.ref(users + userGetKey).on('child_added', function(data) {
			if (data.val().key != undefined && userGetKey == data.val().key && data.val().status == uniqNum) {
				database.ref(users + userGetKey + sub + credentials).update({
			        status: newStatus
			    });
			    status = true;
